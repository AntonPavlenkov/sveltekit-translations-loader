...
function generateTranslationsCode(keysArray: string[], isDevelopment: boolean = false): string {
	// Don't generate code if there are no keys
...
function injectLoadedTranslations(loadFunctionCode: string): {
	...
	if (loadFunctionCode.includes('_loadedTranslations')) {
		return { success: true, code: loadFunctionCode };
	}

	// Check if _getTranslations and _translationKeys are available
	if (
		!loadFunctionCode.includes('_getTranslations') ||
		!loadFunctionCode.includes('_translationKeys')
	) {
		return {
			success: false,
			code: loadFunctionCode,
			warning:
				'⚠️  Could not inject _loadedTranslations: missing _getTranslations or _translationKeys imports'
		};
	}
			...
			if (trimmedLine.includes('return {}') || trimmedLine.includes('return { }')) {
				...
				if (braceCount !== 0) {
					return {
						...
						code: loadFunctionCode,
						warning:
							'⚠️  Could not inject _loadedTranslations: incomplete return statement (unmatched braces)'
					};
				}
	...
	if (!foundLoadFunction) {
		return {
			success: false,
			code: loadFunctionCode,
			warning: '⚠️  Could not inject _loadedTranslations: no load function found'
		};
	}
	...
	if (!foundReturnStatement) {
		return {
			...
			code: loadFunctionCode,
			warning:
				'⚠️  Could not inject _loadedTranslations: no return statement found in load function'
		};
	}
...
export function injectTranslationKeys(
	...
	if (keysArray.length === 0 && !existsSync(serverFilePath)) {
		if (verbose) {
			console.log(
				`⏭️  Skipping ${serverFilePath} for route ${routePath} - no keys and no existing file`
			);
		}
		return;
	}
	...
	if (existsSync(serverFilePath)) {
		...
		if (existingContent.includes('export const load')) {
			...
			// If there are no keys, just remove the auto-generated code and clean up _loadedTranslations
			if (keysArray.length === 0) {
				// Remove _loadedTranslations from return statements
				newContent = newContent
					...
					.replace(/{\s*,\s*}/g, '{}'); // Clean up empty objects with trailing commas
				...
				if (!injectionResult.success) {
					if (verbose) {
						console.warn(`⚠️  ${injectionResult.warning}`);
					}
	...
	if (verbose) {
		...
		console.log(`✅ ${action} ${fileType} for route ${routePath} ${keyInfo}`);
	}
}
