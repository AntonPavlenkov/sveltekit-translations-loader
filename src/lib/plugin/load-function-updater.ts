 ...
 */
function generateTranslationsCode(existingContent?: string): { code: string; functionId: string } {
	const importPath = '$lib/@i18n/_generated/server/translations-injector';

	// Try to extract existing _functionId from the file content
	let functionId: string;
	if (existingContent) {
		const functionIdMatch = existingContent.match(/const _functionId = ['"`]([^'"`]+)['"`]/);
		if (functionIdMatch) {
			functionId = functionIdMatch[1];
		} else {
			functionId = Math.random().toString(36).substring(2, 15);
		}
	} else {
		functionId = Math.random().toString(36).substring(2, 15);
	}
...
function buildFileContent(
	...
	console.log('🚀 ~ buildFileContent ~ generatedCode:', generatedCode);
...
function injectLoadedTranslations(loadFunctionCode: string): {
	...
	if (!loadFunctionCode.includes('_getTranslations') || !loadFunctionCode.includes('_fileType')) {
		return {
			...
			warning:
				'⚠️  Could not inject _loadedTranslations: missing _getTranslations or _fileType imports'
		};
	}
